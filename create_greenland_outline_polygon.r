# create shapefile from raster - Greenland style
...
gdal_polygonizeR <- function(x, outshape=NULL, gdalformat = 'ESRI Shapefile', pypath=NULL, readpoly=TRUE, quiet=TRUE) {
    ...
    if (!file.exists(pypath)) stop("Can't find gdal_polygonize.py on your system: if using anaconda python, try `conda install gdal`")
    owd <- getwd()
    on.exit(setwd(owd))
